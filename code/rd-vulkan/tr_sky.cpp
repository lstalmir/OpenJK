...
static void ClearSkyBox( void ) {
	...
	for( i = 0; i < 6; i++ ) {
		...
		sky_mins[0][i] = sky_mins[1][i] = MIN_WORLD_COORD; //-9999;
		sky_maxs[0][i] = sky_maxs[1][i] = MAX_WORLD_COORD; // 9999;
	}
}
...
void RB_SkyFogFinish( void ) {
	static float atmosphereFadeOutTime = 0;
	...
	// constant parameters
	tr.globals.skyParms.atmosphereDensity = 1;
	tr.globals.skyParms.atmosphereDistance = 120;
	tr.globals.skyParms.atmosphereFalloff = 5;

	// slowly fade out the atmosphere if the game stops drawing the skybox
	if( !backEnd.skyRenderedThisView ) {
		if( backEnd.skyOuterbox ) {
			if( atmosphereFadeOutTime == 0 ) {
				// the sky has just disappeared, start counting time to fade out the atmosphere
				atmosphereFadeOutTime = tr.refdef.floatTime;
			}

			float atmosphereDensity = ( 1 - ( tr.refdef.floatTime - atmosphereFadeOutTime ) );
			if( atmosphereDensity <= 0 ) {
				// stop drawing atmosphere
				atmosphereFadeOutTime = 0;
				backEnd.skyOuterbox = NULL;
				return;
			}
			else {
				// update the atmosphere's density
				tr.globals.skyParms.atmosphereDensity = atmosphereDensity;
			}
		}
		else {
			return;
	...
	tr.globals.skyParms.atmosphereDensity *= 0.8f;

	assert( backEnd.skyOuterbox );

	// rebind depth target for reading
...
void RB_StageIteratorSky( void ) {
	...
	if( tess.shader->sky->outerbox ) {
		backEnd.skyOuterbox = tess.shader->sky->outerbox;
