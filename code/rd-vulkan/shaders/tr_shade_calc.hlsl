...
// shade shaders can be specialized to avoid unnecessary operations
[[vk::constant_id( 0 )]] const int r_spec = 0;
...
static float3 GlobalVectorToLocal( float3 vec ) {
	float3 o;
	o.x = dot( vec, tr_view.ori.axis[0] );
	o.y = dot( vec, tr_view.ori.axis[1] );
	o.z = dot( vec, tr_view.ori.axis[2] );
	return o;
}
...
float2 RB_CalcFogTexCoords( float3 v ) {
	...
	localVec = tr_model.ori.origin - tr_view.ori.origin;
	fogDistanceVector.xyz = -tr_view.ori.modelMatrix[2].xyz;
	fogDistanceVector[3] = dot( localVec, tr_view.ori.axis[0] );
	...
	if( fog.hasSurface ) {
		fogDepthVector[0] = fog.surface[0] * tr_view.ori.axis[0][0] +
							fog.surface[1] * tr_view.ori.axis[0][1] + fog.surface[2] * tr_view.ori.axis[0][2];
		fogDepthVector[1] = fog.surface[0] * tr_view.ori.axis[1][0] +
							fog.surface[1] * tr_view.ori.axis[1][1] + fog.surface[2] * tr_view.ori.axis[1][2];
		fogDepthVector[2] = fog.surface[0] * tr_view.ori.axis[2][0] +
							fog.surface[1] * tr_view.ori.axis[2][1] + fog.surface[2] * tr_view.ori.axis[2][2];
		fogDepthVector[3] = -fog.surface[3] + dot( tr_view.ori.origin, fog.surface.xyz );

		eyeT = dot( tr_view.ori.viewOrigin, fogDepthVector.xyz ) + fogDepthVector[3];
	}
	else {
...
static void ComputeTexCoords( vertex_t vertex, out stageVars_t svars ) {
	...
	for( b = 0; b < NUM_TEXTURE_BUNDLES; b++ ) {
		...
		if( (r_spec & TR_SHADER_SPEC_NOTEXMODS) == 0 ) {
			int tm;
...
float RB_CalcSpecularAlpha( float3 position, float3 normal ) {
	...
	viewer = tr_view.ori.viewOrigin - position;
