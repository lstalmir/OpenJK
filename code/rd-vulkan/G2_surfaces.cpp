...
const surfaceInfo_t *G2_FindOverrideSurface(int surfaceNum,const surfaceInfo_v &surfaceList)
{
	...
	if (surfaceNum<0)
	{
		...
		for(size_t i=0; i<surfaceList.size(); i++)
		{
			...
			{
				QuickOverride.Set(surfaceList[i].surface,(int)i);
			}
		}
		return NULL;
	}
...
int G2_IsSurfaceLegal(const model_s *mod_m, const char *surfaceName, uint32_t *flags)
{
	...
	for( int i = 0; i < mod_m->mdxm->header->numSurfaces; i++ )
	{
							 ...
							 int *surfIndex/*NULL*/)
{
	...
	const mdxmHierarchyOffsets_t *surfIndexes = (mdxmHierarchyOffsets_t *)((byte *)ghlInfo->currentModel->mdxm->header + sizeof(mdxmHeader_t));
	...
	for (i = (int)slist.size() - 1; i >= 0; i--)
	{
		...
		{
			const mdxmSurface_t	*surf = ((trMdxmSurface_t *)G2_FindSurface(ghlInfo->currentModel, slist[i].surface, 0))->header;
...
void G2_FindRecursiveSurface(const model_t *currentModel, int surfaceNum, surfaceInfo_v &rootList, int *activeSurfaces)
{
	...
	int							i;
 	const mdxmSurface_t				*surface = ((trMdxmSurface_t *)G2_FindSurface(currentModel, surfaceNum, 0))->header;
...
int G2_GetParentSurface(CGhoul2Info *ghlInfo, const int index)
{
	...
	const mdxmSurface_t				*surf = ((trMdxmSurface_t *)G2_FindSurface(ghlInfo->currentModel, index, 0))->header;
	const mdxmSurfHierarchy_t		*surfInfo = (mdxmSurfHierarchy_t *)((byte *)surfIndexes + surfIndexes->offsets[surf->thisSurfaceIndex]);
